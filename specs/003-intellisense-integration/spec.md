...
**Independent Test**: Activate the extension with a valid compile-commands artifact for the selected model and component, switch active build context values, and verify that IntelliSense updates to the newly selected context without using another artifact as fallback.

**Acceptance Scenarios**:

1. **Given** a valid active configuration whose expected compile-commands artifact exists, **When** the extension activates or the active model or component changes, **Then** IntelliSense updates using the compile database for that exact active configuration.
...
3. **Given** `tfTools.artifactsPath` changes, **When** refresh completes, **Then** the `Compile Commands` row recomputes its state for the new expected path of the active configuration.
...
- The active compile-commands artifact is missing for the current model and component even though another compile database exists elsewhere under the artifacts root.
...
- **FR-001**: The system MUST resolve the expected compile-commands artifact for the active configuration from `tfTools.artifactsPath`, the active model id, and the active component id.
- **FR-002**: The system MUST treat the resolved compile-commands artifact for the active configuration as the only valid IntelliSense source for that configuration and MUST NOT fall back to a different artifact path, model, component, or previously applied IntelliSense state.
...
- **FR-005**: The system MUST expose a user-invokable IntelliSense refresh action once this slice is implemented.
...
- **Active Compile-Commands Artifact**: The expected compile database for the currently selected model and component, including its resolved path and whether it is present.
...
- Source of truth inputs: `tfTools.artifactsPath`, the active model/component selection already managed by earlier slices, the manifest path and content when they affect active configuration normalization, workspace folder state, extension/provider availability, and the active compile-commands artifact on disk.
...
- **SC-001**: In validation testing across representative model and component changes, 100% of successful IntelliSense refreshes use the compile-commands artifact expected for the active configuration rather than a fallback artifact.
...
- The active compile-commands artifact continues to use the path pattern `<artifacts-root>/<model-id>/<component-id>.cc.json`.
