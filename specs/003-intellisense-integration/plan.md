...
**Storage**: Workspace state for active model, target, and component selection; resource-scoped VS Code settings for `tfTools.artifactsPath`, `tfTools.manifestPath`, and `C_Cpp.default.configurationProvider`; manifest file fields for `artifactFolder`, `artifactName`, and `artifactSuffix`; output-channel logs for persistent warning and missing-artifact reporting
