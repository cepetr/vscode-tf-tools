...
- Update `package.json` and `src/extension.ts` together so `tfTools.flash` and `tfTools.upload` are contributed commands, appear in the Command Palette only when applicable, and remain scoped out of standard build-task entry points; enforce with integration tests covering `menus.commandPalette`, tree-item action visibility, and updated scope-guard expectations in `src/test/integration/configuration-scope.integration.test.ts`.
