...
- [X] T031 [US3] Wire status-bar updates and restore-on-activation behavior in `src/extension.ts`
- [X] T032 [US3] Handle manifest-change re-normalization for restored selections in `src/manifest/manifest-service.ts` and `src/extension.ts`
