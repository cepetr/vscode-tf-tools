...
2. **Given** status-bar visibility is enabled, **When** the active selection changes, **Then** the status bar updates to show the current model, target, and component without requiring a reload.
...
- **FR-011**: The status-bar surface MUST display the active model, target, and component in a stable, human-readable format and update whenever the active selection changes.
...
- **FR-016**: This feature MUST not expose build execution, build-option editing, build-artifact status, IntelliSense state, excluded-file markers, flash/upload actions, or debug launch controls.
