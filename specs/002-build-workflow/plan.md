...
- Keep `Build` visible but disabled in the primary header and also visible but disabled in the overflow menu, and keep `Clippy`, `Check`, and `Clean` visible but disabled in the overflow menu through `package.json`, `src/extension.ts`, and new workflow command modules, enforced by integration tests that inspect contributed title-area actions and blocked execution behavior.
