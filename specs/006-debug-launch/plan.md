...
Implement Debug Launch by replacing the old top-level debug-profile model with component-scoped debug entries, deriving the executable artifact path from the selected component and target instead of from manifest-provided executable strings, exposing the informal-spec debug token set during template substitution, and resolving the first matching debug entry in declaration order. The work stays within the existing VS Code extension architecture: manifest parsing and validation move into `src/manifest/`, executable-state derivation stays in `src/intellisense/artifact-resolution.ts`, debug launch orchestration stays in `src/commands/debug-launch.ts`, UI state remains driven from `src/extension.ts` and `src/ui/configuration-tree.ts`, and tests and fixtures are updated to the hard-cutover schema with no legacy compatibility layer.
