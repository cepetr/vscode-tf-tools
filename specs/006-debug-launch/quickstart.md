...
Verify Debug Launch end to end in a VS Code Extension Development Host using the revised manifest contract: component-scoped debug entries, optional `when` match-all behavior, declaration-order selection, derived executable-row state, visible-but-disabled Start Debugging surfaces when blocked, conditional Command Palette visibility, invocation-time template loading, `${tfTools.debugProfileName}` substitution, debug API launch, and explicit failure plus logging behavior.

## Prerequisites

- Install repository dependencies and compile the extension.
- Update fixture manifests and workspaces so they cover:
  - a selected component whose first matching debug entry launches successfully
  - a selected component whose first entry omits `when` and therefore matches all contexts
  - multiple matching entries where declaration order decides the selected entry
  - no matching debug entry on the selected component
  - a matching debug entry whose derived executable is missing
  - a matching debug entry whose template file is missing
  - a matching debug entry whose template contains invalid JSONC
  - a matching debug entry whose variables include an unknown token or a cycle
...
## Scenario 1: The first matching component debug entry launches from every supported surface
...
## Scenario 2: Declaration order decides among multiple matching debug entries

1. Start with a manifest where the selected component contains two or more debug entries whose `when` expressions all match the active context.
2. Place a different template in the first matching entry than in later matching entries.
...
5. Confirm the launched configuration uses the first matching entry's template and `${tfTools.debugProfileName}` value.

## Scenario 3: Omitted `when` acts as a component-wide default entry

1. Configure a selected component with a first debug entry whose `when` is omitted.
2. Switch across multiple models or targets for that same component.
3. Confirm the entry continues to match all active contexts for that component unless a still-earlier matching entry exists.
...
1. Switch to a context where the selected component has no matching debug entry.
...
4. Switch to a context where a debug entry matches but the derived executable file does not exist.
...
1. Start with a matching debug entry and a target that defines `artifactSuffix` and `executableExtension`.
...
1. Start with a matching debug entry whose derived executable exists but whose template file is missing.
...
4. Invoke `Trezor: Start Debugging` with a matching debug entry.
...
1. Configure a debug entry whose `template` path attempts to escape `tfTools.debug.templatesPath`.
...
- Unit tests cover manifest parsing for component-scoped debug entries, omitted-`when` match-all semantics, declaration-order resolution, derived executable naming, JSONC template parsing, root-traversal rejection, `${tfTools.debugProfileName}` substitution, and cycle detection.
