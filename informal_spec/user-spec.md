...
- Missing templates or malformed template content shall not by themselves disable the visible `Start Debugging` actions before invocation when exactly one valid debug profile is resolved and the executable artifact exists.
...
- If exactly one valid debug profile is resolved and the executable artifact exists, missing templates or malformed template content shall not by themselves disable the visible `Start Debugging` actions before invocation.
...
- If `Trezor: Start Debugging` appears in the Command Palette, it shall do so only when exactly one valid debug profile is resolved for the active build context and the executable artifact exists.
