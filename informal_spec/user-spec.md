...
- After Build Workflow is implemented, the Configuration view shall keep `Build` as the only primary view-title action and shall place `Run Clippy`, `Run Check`, and `Run Clean` in the view overflow menu.
- After IntelliSense Integration is implemented, `Refresh IntelliSense` shall be placed in the view overflow menu after `Run Clippy`, `Run Check`, and `Run Clean`, rather than added as another primary view-title action.
